...
class Engine {
    ...
    */
    ...
    EncodeMove(from: Square|number, to: Square|number, type: MoveType|number) {
     ...
     * Filters out any pseudo‐legal moves that leave the king in check.
     */
    GenerateLegalMoves(capturesOnly: boolean = false): number[] {
        const legalMoves: number[] = [];
        const side = this.BoardState.SideToMove;
        const enemy = side ^ 1;

        const pseudoMoves = this.GenerateMoves(capturesOnly);

        for (const move of pseudoMoves) {
            // 2) Try it on the board
            if (!this.MakeMove(move)) {
                this.UnmakeMove(move)
                // illegal by construction (e.g. moving off‐board, promotions etc.)
                continue;
            }

            // 3) Find where our king ended up
            const kingBB = this.BoardState.PiecesBB[PieceType.King + 6 * side];
            const kingSquare = this.GetLS1B(kingBB);

            // 4) If that square is attacked by the enemy, it’s *not* legal
            if (!this.IsSquareAttacked(kingSquare, enemy)) {
    ...
    isGameOver(legalMovesLength: number) {
        const side = this.BoardState.SideToMove;
        const enemy = side ^ 1;

        const kingSquare = this.GetLS1B(this.BoardState.PiecesBB[PieceType.King + 6 * side]);
        const inCheck = this.IsSquareAttacked(kingSquare, enemy);
    ...
    Search(targetDepth: number) {
        ...
        // console.log(`bestmove ${this.StringifyMove(this.pvArray[0][0])}`);
           // console.log(`bestmove ${this.pvArray[0][0]}`);
        return this.pvArray[0][0]
    }
